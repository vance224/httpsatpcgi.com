 ...
 * @return array
 */
function architecturedesigner_body_classes( $classes ) {
...
add_filter( 'body_class', 'architecturedesigner_body_classes' );
...
if (!function_exists('architecturedesigner_str_replace_assoc')) {

    /**
     * architecturedesigner_str_replace_assoc
     * @param  array $replace
     * @param  array $subject
     * @return array
     */
    function architecturedesigner_str_replace_assoc(array $replace, $subject) {
 ...
 */
function architecturedesigner_add_to_cart_fragment( $fragments ) {
	...
	<a class="nav-link cart-icon-wrap header-cart" href="javascript:void(0);" role="button" data-bs-toggle="collapse" data-bs-target="#Cart">
		...
		?>	
			<span><?php echo esc_html_e('0','architecture-designer'); ?></span>
		<?php
		}
		?>
	</a>
...
add_filter( 'woocommerce_add_to_cart_fragments', 'architecturedesigner_add_to_cart_fragment' );
...
if ( ! function_exists( 'architecturedesigner_breadcrumb_title' ) ) {
	function architecturedesigner_breadcrumb_title() {
		...
		elseif ( is_day() ) : 
			...
			printf( __( 'Daily Archives: %s', 'architecture-designer' ), get_the_date() );
		
		elseif ( is_month() ) :
		
			printf( __( 'Monthly Archives: %s', 'architecture-designer' ), (get_the_date( 'F Y' ) ));
			
		elseif ( is_year() ) :
		
			printf( __( 'Yearly Archives: %s', 'architecture-designer' ), (get_the_date( 'Y' ) ) );
			
		elseif ( is_category() ) :
		
			printf( __( 'Category Archives: %s', 'architecture-designer' ), (single_cat_title( '', false ) ));

		elseif ( is_tag() ) :
		
			printf( __( 'Tag Archives: %s', 'architecture-designer' ), (single_tag_title( '', false ) ));
			
		elseif ( is_404() ) :

			printf( __( 'Error 404', 'architecture-designer' ));
			
		elseif ( is_author() ) :
		
			printf( __( 'Author: %s', 'architecture-designer' ), (get_the_author( '', false ) ));		
...
function architecturedesigner_breadcrumbs() {
	
	$showOnHome	= esc_html__('1','architecture-designer'); 	// 1 - Show breadcrumbs on the homepage, 0 - don't show
	$delimiter 	= '';   // Delimiter between breadcrumb
	$home 		= esc_html__('Home','architecture-designer'); 	// Text for the 'Home' link
	$showCurrent= esc_html__('1','architecture-designer'); // Current post/page title in breadcrumb in use 1, Use 0 for don't show
	$before		= '<li class="active">'; // Tag before the current Breadcrumb
	$after 		= '</li>'; // Tag after the current Breadcrumb
	$seprator	= get_theme_mod('seprator','<span style="color: #f9c12c;font-size: 14px;margin: 0 10px;">→</span>');
	global $post;
	$homeLink = home_url();
    ...
    if ( is_category() ) 
	{
		...
		echo $before . esc_html__('Archive by category','architecture-designer').' "' . esc_html(single_cat_title('', false)) . '"' .$after;
		
	} 
	
	elseif ( is_search() ) 
	{
		echo $before . esc_html__('Search results for','architecture-designer').' "' . esc_html(get_search_query()) . '"' . $after;
	} 
	...
	elseif ( is_tag() ) 
	{
		echo $before . esc_html__('Posts tagged ','architecture-designer').' "' . esc_html(single_tag_title('', false)) . '"' . $after;
	} 
	
	elseif ( is_author() ) {
		global $author;
		$userdata = get_userdata($author);
		echo $before . esc_html__('Articles posted by ','architecture-designer').'' . $userdata->display_name . $after;
	} 
	
	elseif ( is_404() ) {
		echo $before . esc_html__('Error 404 ','architecture-designer'). $after;
    }
	
    if ( get_query_var('paged') ) {
		if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo '';
		echo ' ( ' . esc_html__('Page','architecture-designer') . '' . esc_html(get_query_var('paged')). ' )';
		if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo '';
    }
...
add_action( 'wp_ajax_architecturedesigner_dismissed_notice_handler', 'architecturedesigner_ajax_notice_handler' );

/**
 * AJAX handler to store the state of dismissible notices.
 */
function architecturedesigner_ajax_notice_handler() {
    if ( isset( $_POST['type'] ) ) {
...
add_action( 'wp_ajax_install_act_plugin', 'architecturedesigner_admin_install_plugin' );

function architecturedesigner_admin_install_plugin() {
    /**
     * Install Plugin.
     */
